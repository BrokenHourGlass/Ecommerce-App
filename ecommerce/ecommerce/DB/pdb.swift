...
class ProductDB{
    ...
    func getItemCategory(category: String) -> [NewProduct]{
        let query = "select * from product where category = '\(category)';"
        var stmt: OpaquePointer?
        var prod = [NewProduct]()
        
        print(sqlite3_prepare(pointer, query, -1, &stmt, nil))
        
        if (sqlite3_prepare(pointer, query, -1, &stmt, nil) != 0){
            while(sqlite3_step(stmt) == SQLITE_ROW){
                let id = sqlite3_column_int(stmt, 0)
                let name = String(cString: sqlite3_column_text(stmt, 1))
                let category = String(cString: sqlite3_column_text(stmt, 2))
                ...
                prod.append(NewProduct(id: Int(id), name: name, category: category, new: (new != 0), price: Int(price), featured: (featured != 0), cartIMG: cartIMG, productIMG: productIMG, description: description, features: features, contents: [NewItem](), previews: [NewPreview](), recommended: [NewRecommended]()))
                
            }
        }
        else{
            print("error in query")
            print(String(cString: sqlite3_errmsg(pointer)))
        }
        return prod
